...
export default function AdminProfitAnalysis({ selectedShop = null }) {
  ...
  const fetchData = useCallback(async (searchVal) => {
    ...
    try {
      ...
      const { data: res } = await billingAdminAPI.getProfitAnalysis(params)
      setData(res)
    } catch {
    ...
    <div className="space-y-4">
      ...
      {data && (
        <>
          ...
          {activeSection === 'overview' && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-3">
                ...
                <KPICard label="Net Profit" value={fmt(s?.net_profit)} sub={pct(s?.profit_margin) + ' margin'} icon={isProfit ? TrendingUp : TrendingDown} color={isProfit ? 'bg-green-500' : 'bg-red-500'} />
                ...
                <div className="bg-white rounded-xl border border-slate-200 p-4 shadow-sm">
                  ...
                  <ResponsiveContainer width="100%" height={200}>
                    <AreaChart data={data.daily_pnl}>
                      ...
                      <Area type="monotone" dataKey="revenue" name="Revenue" stroke="#6366f1" fill="url(#aRev)" strokeWidth={2} />
                      <Area type="monotone" dataKey="expenses" name="Expenses" stroke="#f59e0b" fill="url(#aExp)" strokeWidth={2} />
                    </AreaChart>
                  </ResponsiveContainer>
                </div>
              )}
                  ...
                  <div className="bg-white rounded-xl border border-slate-200 p-4 shadow-sm">
                    ...
                    <ResponsiveContainer width="100%" height={280}>
                      <AreaChart data={data.daily_pnl}>
                        ...
                        <Tooltip content={<CustomTooltip />} />
                        <Legend wrapperStyle={{ fontSize: 11 }} />
                        <Area type="monotone" dataKey="revenue" name="Revenue" stroke="#6366f1" fill="url(#tRev)" strokeWidth={2} />
                        <Area type="monotone" dataKey="expenses" name="Expenses" stroke="#f59e0b" fill="url(#tExp)" strokeWidth={2} />
                        <Area type="monotone" dataKey="net_profit" name="Net Profit" stroke="#22c55e" fill="url(#tProfit)" strokeWidth={2} />
                      </AreaChart>
                    </ResponsiveContainer>
                  </div>

                  <div className="bg-white rounded-xl border border-slate-200 p-4 shadow-sm">
                    <h3 className="text-sm font-semibold text-slate-700 mb-3">Daily Bills & Discounts</h3>
                ...
                <div className="bg-white rounded-xl border border-slate-200 p-4 shadow-sm">
                  ...
                  {data.expense_breakdown?.length > 0 ? (
                    <>
                      <ResponsiveContainer width="100%" height={220}>
                        <PieChart>
                          <Pie data={data.expense_breakdown} dataKey="amount" nameKey="category" cx="50%" cy="50%" outerRadius={80}
                ...
                <div className="bg-white rounded-xl border border-slate-200 p-4 shadow-sm">
                  ...
                  {data.payment_split && (
                    <>
                      ...
                      <div className="space-y-1.5 mt-2">
                        {[
                          { label: 'Cash', value: data.payment_split.cash, pctV: data.payment_split.cash_pct, color: '#6366f1' },
                          { label: 'Card', value: data.payment_split.card, pctV: data.payment_split.card_pct, color: '#22c55e' },
                          { label: 'Online', value: data.payment_split.online, pctV: data.payment_split.online_pct, color: '#06b6d4' },
                          ...
                          <div key={i} className="flex items-center justify-between text-xs">
                            ...
                            <div className="flex gap-3">
                              ...
                              <span className="text-slate-400 w-10">{pct(d.pctV)}</span>
                            </div>
                          </div>
                        ))}
                      </div>
          ...
          {activeSection === 'shops' && (
            <div className="space-y-4">
              {data.shop_comparison?.length > 0 ? (
                <>
                  <div className="bg-white rounded-xl border border-slate-200 p-4 shadow-sm">
                    <h3 className="text-sm font-semibold text-slate-700 mb-3">Revenue & Profit by Shop</h3>
                    <ResponsiveContainer width="100%" height={280}>
                      <BarChart data={data.shop_comparison}>
                        <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" />
                  ...
                  <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                    <div className="p-4 border-b border-slate-100 flex items-center gap-2">
                      ...
                      <h3 className="text-sm font-semibold text-slate-700">Shop-wise P&L Summary</h3>
                    </div>
                    <div className="overflow-x-auto">
                      <table className="w-full text-xs">
                        <thead className="bg-slate-50">
                          <tr>
                            {['Shop', 'Bills', 'Revenue', 'Expenses', 'Net Profit', 'Margin'].map(h => (
                              <th key={h} className="text-left py-2.5 px-3 text-slate-500 font-medium whitespace-nowrap">{h}</th>
                            ))}
                          </tr>
                        </thead>
                        <tbody>
                          {data.shop_comparison.map((shop, i) => (
                            <tr key={i} className="border-b border-slate-50 hover:bg-slate-50">
                              <td className="py-2 px-3 font-medium text-slate-700">{shop.shop_name}</td>
                              <td className="py-2 px-3 text-slate-600">{shop.bills}</td>
                              <td className="py-2 px-3 text-indigo-600 font-semibold">{fmt(shop.revenue)}</td>
                              <td className="py-2 px-3 text-orange-600">{fmt(shop.expenses)}</td>
                              <td className={`py-2 px-3 font-semibold ${shop.net_profit >= 0 ? 'text-green-600' : 'text-red-500'}`}>{fmt(shop.net_profit)}</td>
                              <td className={`py-2 px-3 ${shop.profit_margin >= 0 ? 'text-green-600' : 'text-red-500'}`}>{pct(shop.profit_margin)}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
                ...
                <div className="bg-white rounded-xl border border-slate-200 p-12 text-center">
                  ...
                  <p className="text-slate-400">
                    {selectedShop ? 'Shop filter is active — deselect shop to see comparison' : 'No multi-shop data available'}
                  </p>
                </div>
              )}
            </div>
                          ...
                          {data.top_items.map((item, i) => (
                            <tr key={i} className="border-b border-slate-50 hover:bg-slate-50">
                              <td className="py-2 px-3">
                                <span className="w-5 h-5 rounded-full flex items-center justify-center text-white text-[10px] font-bold inline-flex" style={{ background: COLORS[i % COLORS.length] }}>{i + 1}</span>
                              </td>
