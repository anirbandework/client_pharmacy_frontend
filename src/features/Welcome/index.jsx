...
import { ArrowRight, ArrowLeft, Lock, Package, Shield, Eye, EyeOff, Info, Building2, Briefcase, Bell, Database, Zap } from 'lucide-react'
  ...
  return (
    <div className="fixed inset-0 overflow-y-auto" style={{ background: '#010c1a' }}>
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        <div className="absolute inset-0" style={{ background: 'radial-gradient(ellipse 90% 65% at 50% -10%, rgba(17,43,99,0.55) 0%, transparent 68%)' }} />
        <div className="absolute inset-0" style={{ background: 'radial-gradient(ellipse 60% 50% at 85% 90%, rgba(29,20,85,0.4) 0%, transparent 60%)' }} />
        <div className="absolute inset-0" style={{ background: 'radial-gradient(ellipse 50% 40% at 10% 80%, rgba(15,30,70,0.35) 0%, transparent 55%)' }} />
        ...
        {/* Animated orbs */}
        <div className="orb-1 absolute top-[5%] left-[8%] w-[520px] h-[520px] rounded-full" style={{ background: 'radial-gradient(circle, rgba(59,130,246,0.13) 0%, transparent 68%)', filter: 'blur(45px)' }} />
        <div className="orb-2 absolute bottom-[5%] right-[5%] w-[620px] h-[620px] rounded-full" style={{ background: 'radial-gradient(circle, rgba(99,102,241,0.11) 0%, transparent 68%)', filter: 'blur(55px)' }} />
        <div className="orb-3 absolute top-[38%] left-[45%] w-[440px] h-[440px] rounded-full" style={{ background: 'radial-gradient(circle, rgba(139,92,246,0.09) 0%, transparent 68%)', filter: 'blur(65px)' }} />
      ...
      <div className="relative z-10 min-h-screen flex flex-col">
        ...
        <header
          ...
          style={{ background: 'rgba(1,12,26,0.6)', backdropFilter: 'blur(20px)', borderBottom: '1px solid rgba(255,255,255,0.07)' }}
        >
          <div className="w-full px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between">
            ...
            <div className={`flex items-center gap-3 sm:gap-5 ${mounted ? 'appear-r' : 'opacity-0'}`} style={{ animationDelay: '0.2s' }}>
              <p className="hidden sm:block text-xs text-slate-500">
                powered by <span className="text-sm font-semibold text-slate-300">Indus Infotech</span>
              </p>
          ...
          <div className="w-full max-w-6xl grid lg:grid-cols-2 gap-10 lg:gap-16 items-center">
            ...
            >
              ...
              <div
                ...
                style={{
                  background: 'linear-gradient(145deg, rgba(10,20,46,0.85) 0%, rgba(5,12,32,0.92) 100%)',
              ...
              >
                ...
                <div className="mb-7 text-center">
                  <p className="text-xs font-semibold tracking-widest text-slate-500 uppercase mb-1">Welcome Back</p>
                  <h3 className="text-xl font-bold text-white">Sign in to LedgerX</h3>
                </div>

                {/* Role selector — sliding pill */}
                <div className="mb-6">
                  <p className="text-center text-[10px] font-semibold tracking-widest text-slate-600 uppercase mb-3">Select Your Role</p>
                  <div
                    className="relative grid grid-cols-3 p-1 rounded-2xl"
                    style={{ background: 'rgba(2,8,20,0.7)', border: '1px solid rgba(255,255,255,0.06)' }}
                  >
                    {/* Sliding pill */}
                    <div
                    ...
                    {ROLES.map(({ key, label, sub, Icon }) => (
                      ...
                      >
                        <Icon className={`w-4.5 h-4.5 mb-1 transition-colors duration-200 ${loginType === key ? 'text-blue-400' : 'text-slate-600 group-hover:text-slate-400'}`} style={{ width: '18px', height: '18px' }} />
                        <span className={`text-xs font-semibold transition-colors duration-200 ${loginType === key ? 'text-white' : 'text-slate-500 group-hover:text-slate-400'}`}>{label}</span>
                        <span className="text-[10px] text-slate-600 leading-none mt-0.5">{sub}</span>
                      </button>
                    ))}
                  </div>
                </div>
                  ...
                  <form onSubmit={handleSendOTP} className="step-enter space-y-4">
                    ...
                    <div>
                      ...
                      <div className="relative">
                        <span className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-500 text-sm pointer-events-none select-none">+91</span>
                        <input
                          type="tel"
                          value={phone}
                          onChange={e => setPhone(e.target.value)}
                          placeholder="9800000000"
                          className="input-field w-full pl-14 pr-4 py-3 rounded-xl text-white placeholder-slate-600 text-sm"
                          required
                        />
                      </div>
                    </div>
                        ...
                        <div>
                          ...
                          <div className="relative">
                            <input
                              ...
                              className="input-field w-full px-4 py-3 pr-11 rounded-xl text-white placeholder-slate-600 text-sm"
                              required
                            />
                            <button
                              type="button"
                ...
                {step === 'otp' && (
                  <form onSubmit={handleVerifyOTP} className="step-enter space-y-5">
                    <div className="text-center">
                      ...
                      <p className="text-slate-500 text-sm">Code sent to <span className="text-slate-300">{phone}</span></p>
                      <button
                        type="button"
                        onClick={resetForm}
                    ...
                    <div className="text-center">
                      {countdown > 0 ? (
                        <p className="text-slate-500 text-sm">Resend OTP in <span className="text-slate-300 font-medium tabular-nums">{countdown}s</span></p>
                      ) : (
        ...
        <footer
          ...
          style={{ background: 'rgba(1,12,26,0.6)', backdropFilter: 'blur(20px)', borderTop: '1px solid rgba(255,255,255,0.06)' }}
        >
