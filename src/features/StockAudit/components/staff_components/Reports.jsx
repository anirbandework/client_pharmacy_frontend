  ...
  const handleExport = async () => {
    try {
      ...
      if (activeReport === 'low-stock') {
        ...
        response = await staffStockAuditAPI.exportAuditRecords({ days: appliedDaysAhead })
      } else if (activeReport === 'discrepancies') {
        response = await staffStockAuditAPI.exportAdjustments({ days: 30 })
      }
  ...
  return (
    <div className="space-y-4">
      <div className="flex gap-2 flex-wrap items-center">
        <button onClick={() => setActiveReport('low-stock')} className={`px-4 py-2 rounded-lg font-medium transition-all ${activeReport === 'low-stock' ? 'bg-gradient-to-r from-blue-600 to-indigo-600 text-white shadow-lg shadow-blue-500/20' : 'bg-white dark:bg-slate-800/80 text-gray-700 dark:text-slate-300 hover:bg-gray-50 dark:hover:bg-slate-700/30 border border-slate-200 dark:border-slate-700/50'}`}>Low Stock</button>
        <button onClick={() => setActiveReport('expiring')} className={`px-4 py-2 rounded-lg font-medium transition-all ${activeReport === 'expiring' ? 'bg-gradient-to-r from-blue-600 to-indigo-600 text-white shadow-lg shadow-blue-500/20' : 'bg-white dark:bg-slate-800/80 text-gray-700 dark:text-slate-300 hover:bg-gray-50 dark:hover:bg-slate-700/30 border border-slate-200 dark:border-slate-700/50'}`}>Expiring Items</button>
        <button onClick={() => setActiveReport('discrepancies')} className={`px-4 py-2 rounded-lg font-medium transition-all ${activeReport === 'discrepancies' ? 'bg-gradient-to-r from-blue-600 to-indigo-600 text-white shadow-lg shadow-blue-500/20' : 'bg-white dark:bg-slate-800/80 text-gray-700 dark:text-slate-300 hover:bg-gray-50 dark:hover:bg-slate-700/30 border border-slate-200 dark:border-slate-700/50'}`}>Discrepancies</button>
        
        {activeReport === 'low-stock' && (
          <div className="flex items-center gap-2 ml-4 bg-white dark:bg-slate-800/80 px-3 py-2 rounded-lg border border-slate-200 dark:border-slate-700/50">
            <label className="text-sm font-medium text-gray-700 dark:text-slate-300">Threshold:</label>
            <input type="number" value={threshold} onChange={(e) => setThreshold(e.target.value)} className="w-20 px-2 py-1 border border-gray-300 dark:border-slate-600 rounded text-gray-900 dark:text-white dark:bg-slate-700 focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
        ...
        {activeReport === 'expiring' && (
          <div className="flex items-center gap-2 ml-4 bg-white dark:bg-slate-800/80 px-3 py-2 rounded-lg border border-slate-200 dark:border-slate-700/50">
            ...
            <input type="number" value={daysAhead} onChange={(e) => setDaysAhead(e.target.value)} className="w-20 px-2 py-1 border border-gray-300 dark:border-slate-600 rounded text-gray-900 dark:text-white dark:bg-slate-700 focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
            <button onClick={handleApplyFilter} className="px-3 py-1 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded text-sm hover:from-blue-700 hover:to-indigo-700 shadow-lg shadow-blue-500/20 transition-all">Apply</button>
          </div>
        )}
      </div>
      <div className="bg-white dark:bg-slate-800/80 rounded-xl shadow-lg p-4 md:p-6 border border-slate-200 dark:border-slate-700/50">
        <div className="flex justify-between items-center mb-4">
          <div>
            ...
            {activeReport === 'low-stock' && (
              <p className="text-sm text-gray-500 dark:text-slate-500 mt-1">Showing items with quantity below {appliedThreshold}</p>
            )}
            {activeReport === 'expiring' && (
              <p className="text-sm text-gray-500 dark:text-slate-500 mt-1">Showing items expiring within {appliedDaysAhead} days</p>
            )}
          </div>
            ...
            <div className="space-y-3">
              {data.length === 0 ? (
                <div className="text-center py-12 bg-gray-50 dark:bg-slate-700/30 rounded-xl">
                  ...
                  <p className="text-gray-500 dark:text-slate-500 text-sm md:text-base">No items found</p>
                </div>
              ) : (
                data.map((item, idx) => {
                  const itemData = item.item || item;
                  return (
                    <div key={idx} className="bg-gradient-to-br from-gray-50 to-slate-50 dark:from-slate-800/60 dark:to-slate-700/40 border-2 border-slate-200 dark:border-slate-700/50 rounded-xl p-4 hover:shadow-lg transition-all">
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1">
                          <h4 className="font-bold text-gray-800 dark:text-white mb-2">{itemData.product_name || itemData.item_name}</h4>
                          <div className="grid grid-cols-2 md:grid-cols-3 gap-x-4 gap-y-2 text-sm">
                            ...
                            <p className="text-gray-600 dark:text-slate-400"><span className="font-medium">Batch:</span> <span className="font-mono bg-gray-100 dark:bg-slate-700 px-2 py-0.5 rounded">{itemData.batch_number}</span></p>
                            ...
                            {activeReport === 'discrepancies' ? (
                              <>
                                ...
                                <p className="text-gray-600 dark:text-slate-400"><span className="font-medium">Section:</span> {item.section_name}</p>
                                <p className="text-gray-600 dark:text-slate-400"><span className="font-medium">Rack:</span> {item.rack_number}</p>
                                {item.audited_by_staff_name && <p className="text-gray-500 dark:text-slate-500"><span className="font-medium">Audited by:</span> {item.audited_by_staff_name}</p>}
                              </>
                            ) : (
                              <>
                                ...
                                {itemData.last_audit_date && <p className="text-gray-500 dark:text-slate-500 text-xs col-span-2 md:col-span-3"><span className="font-medium">Last Audit:</span> {new Date(itemData.last_audit_date).toLocaleString()}</p>}
                              </>
                            )}
                          </div>
                        </div>
