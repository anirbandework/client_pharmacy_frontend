  ...
  return (
    <div className="space-y-4">
      <div className="bg-white dark:bg-slate-800/80 rounded-xl shadow-lg border border-slate-200 dark:border-slate-700/50 p-4 md:p-6">
        ...
        {section && (
          <div className="mb-4 p-4 bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 rounded-xl border-2 border-blue-200 dark:border-blue-800/40">
            <h3 className="font-bold text-lg text-blue-900 dark:text-blue-300">Auditing Section: {section.section_name}</h3>
            <p className="text-sm text-blue-700 dark:text-blue-400">Code: {section.section_code}</p>
          </div>
        )}
        {items.length > 0 && (
          <div className="space-y-3">
            {items.map((item) => (
              <div key={item.id} className="bg-gradient-to-br from-gray-50 to-slate-50 dark:from-slate-800/60 dark:to-slate-700/40 border-2 border-slate-200 dark:border-slate-700/50 rounded-xl p-4 hover:shadow-lg transition-all">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1">
                    <h4 className="font-bold text-gray-800 dark:text-white text-base md:text-lg mb-2">{item.product_name || 'N/A'}</h4>
                    ...
                    <details className="text-sm">
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-3 gap-x-4 gap-y-2 mt-2 pl-4 border-l-2 border-blue-200">
                        <p className="text-gray-600 dark:text-slate-400"><span className="font-medium">Composition:</span> {item.composition || 'N/A'}</p>
                        <p className="text-gray-600 dark:text-slate-400"><span className="font-medium">Manufacturer:</span> {item.manufacturer || 'N/A'}</p>
                        <p className="text-gray-600 dark:text-slate-400"><span className="font-medium">Batch:</span> <span className="font-mono bg-gray-100 dark:bg-slate-700 px-2 py-0.5 rounded">{item.batch_number || 'N/A'}</span></p>
                        <p className="text-gray-600 dark:text-slate-400"><span className="font-medium">Package:</span> {item.package || 'N/A'}</p>
                        <p className="text-gray-600 dark:text-slate-400"><span className="font-medium">HSN:</span> {item.hsn_code || 'N/A'}</p>
                        <p className="text-gray-600 dark:text-slate-400"><span className="font-medium">Unit:</span> {item.unit || 'N/A'}</p>
                        <p className="text-gray-600 dark:text-slate-400"><span className="font-medium">MRP:</span> {item.mrp || 'N/A'}</p>
                        <p className="text-gray-600 dark:text-slate-400"><span className="font-medium">Unit Price:</span> {item.unit_price ? `₹${item.unit_price}` : 'N/A'}</p>
                        <p className="text-gray-600 dark:text-slate-400"><span className="font-medium">Selling Price:</span> {item.selling_price ? `₹${item.selling_price}` : 'N/A'}</p>
                        <p className="text-gray-600 dark:text-slate-400"><span className="font-medium">Profit Margin:</span> {item.profit_margin ?? 'N/A'}</p>
                        <p className="text-gray-600 dark:text-slate-400"><span className="font-medium">Mfg Date:</span> {item.manufacturing_date ? new Date(item.manufacturing_date).toLocaleDateString() : 'N/A'}</p>
                        ...
                        <p className="text-gray-500 dark:text-slate-500 text-xs col-span-2 md:col-span-3"><span className="font-medium">Last Audit:</span> {item.last_audit_date ? new Date(item.last_audit_date).toLocaleString() : 'Never'}</p>
                      </div>
                    </details>
                  </div>
                  <div className="flex flex-col items-end gap-2">
                    <input 
                      ...
                      className="w-40 px-3 py-2 border-2 border-gray-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 text-center font-bold dark:bg-slate-700 dark:text-white dark:placeholder-slate-400"
                    />
                    <button 
