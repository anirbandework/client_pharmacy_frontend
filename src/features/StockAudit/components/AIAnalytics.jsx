...
import { Brain, TrendingUp, AlertCircle, Lightbulb, Calendar } from 'lucide-react'
...
const AIAnalytics = () => {
  ...
  const fetchAnalytics = async () => {
    ...
    setError(null)
    try {
      const { data: res } = await stockAuditAPI.getAIAnalytics({ days })
      setData(res)
    } catch (error) {
  ...
  if (loading) return <div className="text-center py-8">Loading AI Analytics...</div>
    ...
    <div className="space-y-6">
      ...
      {data.charts && (
        <div className="grid grid-cols-3 gap-4">
          {data.charts.discrepancy_trend && (
            <div className="bg-white p-4 rounded-lg shadow col-span-2">
              <h3 className="font-semibold mb-3">Discrepancy Trend</h3>
              <Line data={formatLineChart(data.charts.discrepancy_trend)} options={{ responsive: true, maintainAspectRatio: true }} />
            </div>
          )}
          {data.charts.section_discrepancies && (
            <div className="bg-white p-4 rounded-lg shadow">
              <h3 className="font-semibold mb-3">Section Discrepancies</h3>
              <Pie data={formatPieChart(data.charts.section_discrepancies)} options={{ responsive: true, maintainAspectRatio: true }} />
            </div>
          )}
          {data.charts.staff_performance && (
            <div className="bg-white p-4 rounded-lg shadow col-span-3">
              <h3 className="font-semibold mb-3">Staff Performance</h3>
              <Bar data={formatBarChart(data.charts.staff_performance)} options={{ responsive: true, maintainAspectRatio: true }} />
            </div>
          )}
        </div>
      )}
