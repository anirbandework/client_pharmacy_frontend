...
const AdminExcelVerification = () => {
  ...
  const fetchUploads = async () => {
    try {
      ...
      const response = await stockAuditAPI.getAdminExcelUploads(params)
  ...
  const fetchSections = async () => {
    try {
      const response = await stockAuditAPI.getAdminSections()
  ...
  const fetchRacks = async () => {
    try {
      const response = await stockAuditAPI.getAdminRacks()
  ...
  const fetchUploadItems = async (uploadId) => {
    try {
      setLoading(true)
      const response = await stockAuditAPI.getAdminUploadItems(uploadId)
  ...
  const handleAdminVerify = async (uploadId, notes) => {
    try {
      await stockAuditAPI.adminVerifyUpload(uploadId, notes)
      fetchUploads()
  ...
  const handleReject = async (uploadId, reason) => {
    try {
      await stockAuditAPI.rejectUpload(uploadId, reason)
      fetchUploads()
      setSelectedUpload(null)
  ...
  const handleDeleteUpload = async (uploadId) => {
    try {
      await stockAuditAPI.deleteUpload(uploadId)
      fetchUploads()
  ...
  const handleUpdateItem = async (itemId, itemData) => {
    try {
      await stockAuditAPI.updateUploadItem(selectedUpload.id, itemId, itemData)
  ...
  const handleDeleteItem = async (itemId) => {
    try {
      await stockAuditAPI.deleteUploadItem(selectedUpload.id, itemId)
      fetchUploadItems(selectedUpload.id)
...
export default AdminExcelVerification