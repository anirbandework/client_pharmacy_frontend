...
const StatCard = ({ label, value, sub, color = 'blue', icon: Icon }) => {
  const palette = {
    ...
    purple: 'bg-purple-50 border-purple-200 text-purple-700 text-purple-500',
  }
    ...
    <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
      ...
      >
        <div className="flex-1 min-w-0">
          ...
          {item.composition && <p className="text-xs text-gray-500 truncate">{item.composition}</p>}
        </div>
        <div className="flex items-center gap-3 ml-3 shrink-0">
          {item.mrp && (
            <span className="text-xs bg-gray-100 text-gray-600 px-2 py-0.5 rounded">MRP ₹{item.mrp}</span>
          )}
          ...
          {item.suppliers.map((s, i) => (
            <div
              ...
              className={`flex items-center justify-between p-3 rounded-lg border ${
                s.badge === 'best'  ? 'bg-green-50 border-green-200' :
                s.badge === 'worst' ? 'bg-red-50   border-red-200'   :
            ...
            >
              ...
              <div className="text-right">
                <p className="text-sm font-bold text-gray-800">₹{s.avg_purchase_price}/unit</p>
                {s.purchase_margin !== null
                  ? <p className={`text-xs font-semibold ${s.badge === 'best' ? 'text-green-600' : s.badge === 'worst' ? 'text-red-500' : 'text-gray-600'}`}>
                      {s.purchase_margin.toFixed(1)}% margin at MRP
                    </p>
                  : <p className="text-xs text-gray-400">MRP not available</p>
                }
              </div>
            </div>
          ))}
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-4">
        <div className="flex items-center gap-2 mb-3">
          ...
          <h2 className="font-bold text-gray-800">Purchase Margin Analysis</h2>
        </div>
        <div className="flex items-center gap-1.5 mb-4 bg-blue-50 border border-blue-200 rounded-lg px-3 py-2">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <div className="flex flex-col gap-1">
            ...
            <ProductAutocomplete
              ...
              className="px-3 py-2 border border-gray-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-purple-300 w-full" />
          </div>
          <div className="flex flex-col gap-1">
            <label className="text-xs font-semibold text-gray-600 uppercase tracking-wide">Composition</label>
          ...
          <div className="flex flex-col gap-1">
            <label className="text-xs font-semibold text-gray-600 uppercase tracking-wide">Invoice From</label>
            <input type="date" value={filters.date_from}
              onChange={e => setFilters(f => ({ ...f, date_from: e.target.value }))}
              className="px-3 py-2 border border-gray-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-purple-300 w-full" />
          </div>
          <div className="flex flex-col gap-1">
            <label className="text-xs font-semibold text-gray-600 uppercase tracking-wide">Invoice To</label>
      ...
      {!loading && !data && (
        <div className="bg-white rounded-xl border border-gray-200 p-12 text-center">
          ...
          <p className="text-gray-500">No data. Ensure invoices are admin-verified.</p>
        </div>
      )}
      ...
      {!loading && data && (
        <>
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-5">
            ...
            {data.distributor_comparison.length === 0 ? (
              <div className="text-center py-8">
                ...
                <p className="text-sm text-gray-500">No products yet with 2+ distributors.</p>
                <p className="text-xs text-gray-400 mt-1">Comparison appears when the same medicine is purchased from different suppliers.</p>
              </div>
            ) : (
              ...
              {data.supplier_breakdown.map((s, idx) => (
                <div key={idx} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg border border-gray-200">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-semibold text-gray-800 text-sm">{s.supplier_name}</p>
                      <p className="text-xs text-gray-500">{s.product_count} products · {s.item_count} items · ₹{fmt(s.total_purchase_value)} spent</p>
                    </div>
                  </div>
                ...
                <div key={idx} className="p-4 bg-gray-50 rounded-lg border border-gray-200">
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex-1 min-w-0 pr-4">
                      ...
                      {p.composition && <p className="text-xs text-gray-500 truncate">{p.composition}</p>}
                    ...
                    <div className="text-right shrink-0">
                      <p className="text-sm font-bold text-gray-800">₹{fmt(p.purchase_value)}</p>
                  ...
                  <div className="grid grid-cols-3 gap-2 text-xs">
                    <div className="bg-white rounded p-2 text-center border border-gray-100">
                      <p className="text-gray-400 mb-0.5">Qty Purchased</p>
                      <p className="font-bold text-gray-700">{p.quantity}</p>
                    </div>
                    <div className="bg-white rounded p-2 text-center border border-gray-100">
                      <p className="text-gray-400 mb-0.5">Potential Profit</p>
                    ...
                    <div className="bg-white rounded p-2 text-center border border-gray-100">
                      <p className="text-gray-400 mb-0.5">Avg Margin</p>
          ...
          {data.summary.items_without_mrp > 0 && (
            <div className="flex items-start gap-2 px-3 py-2 bg-gray-50 border border-gray-200 rounded-xl">
